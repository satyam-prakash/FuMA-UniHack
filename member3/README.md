...
```bash
PYTHONPATH=. ~/.fuma-venv/v1/bin/python -m pytest member3/tests/ -q   # 35 passed
...
├── tests/                          35 tests: pipeline, delivery, end-to-end
...
- **Nothing invented.** Columns with no source (`UPC`, `Warranty`, `Country Of Origin`, ...) stay empty instead of being filled with plausible-looking values.
